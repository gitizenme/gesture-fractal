...
The original form of this wrapper worked by doing a deep copy of all frame
data as soon as you the user requests [controller frame]. This was different
from our Python, Java, and CSharp wrappers, which tend to wrap a corresponding
C++ objects and retrieve data lazily.
...
In due time we may remove the deep copies entirely, or at least outside
of NSArray allocation.
...
Notes:
* Not yet tested for robust memory management
* Not optimally implemented (as indicated most prominently by doing both
  deep copies and thin wrapping)
* Intending to use ObjectiveC properties in a later version
* Does not rely on Automatic Reference Counting (ARC) to ensure
  compatibility with pre-ARC projects
...
@property (nonatomic, assign, readwrite)const LeapVector *xBasis;
@property (nonatomic, assign, readwrite)const LeapVector *yBasis;
@property (nonatomic, assign, readwrite)const LeapVector *zBasis;
@property (nonatomic, assign, readwrite)const LeapVector *origin;
